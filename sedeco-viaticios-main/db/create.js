...
const data = {
    oficio_fecha: "2024-05-15",
    oficio_lugar_comision: "Uruapan",
    oficio_numero: "199",
    persona_area: "financieros",
    persona_nombre: "Ivan Jaramillo"
}
...
async function addDocumentAsync() {
    try {
      ...
      const docRef = await setDoc(doc(db, "oficios", "Oficio_166"), data);
 ...
 // addDocumentThen();
  function addDocumentThen() {
    try {
      // Referencia a la colección donde quieres agregar el documento
      const docRef = doc(db, 'oficios', "Oficio_167");
      
  
      setDoc(docRef, {
        ejemplo: 'Prueba con .then()',
        name: 'Jorge',
